...
def make_acc_dataset(args,files_dict,acc_d_odir):
    if args.acc_peak1 is not None and args.acc_peak2 is not None and args.acc_signal1 is not None and args.acc_signal2 is not None:
        if os.path.exists(f'{acc_d_odir}/total.csv'):
            ...
            if args.mode == 'fast':
                total_region_signal_sampled = total_region_signal.sample(n=args.sample_size,random_state=55).reset_index(drop=True)
                total_region_signal_sampled.to_csv(f'{acc_d_odir}/total_sampled.csv',index=False)
                split_data(total_region_signal_sampled, "_sampled", acc_d_odir)
            else:
                args.mode = 'full'
...
@click.command(
    name="find_driver_in_transition_dw_benchmark",
...
@click.option(
    ...
    show_default=True,
    type=int,
    help="Sample size for chromatin accessibility dataset for fine-tuning ChromBERT.",
)
@click.option(
    "--odir",
    default="./output",
...
def find_driver_in_transition_dw_benchmark(
    exp_tpm1,
    exp_tpm2,
    acc_peak1,
    acc_peak2,
...
if __name__ == "__main__":
    find_driver_in_transition_dw_benchmark()