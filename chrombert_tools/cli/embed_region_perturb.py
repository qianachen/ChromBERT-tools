import os
import click
from types import SimpleNamespace
import numpy as np
import pandas as pd
import chrombert
from chrombert import ChromBERTFTConfig, DatasetConfig
import torch
from tqdm import tqdm
from .utils import resolve_paths, check_files, check_region_file

    
def model_emb_func_perturb(args,files_dict,odir):
    # init datamodule
    data_config = DatasetConfig(
        kind = "GeneralDataset",
        supervised_file = f"{odir}/model_input.tsv",
        hdf5_file = files_dict["hdf5_file"],
        batch_size = 4,
        num_workers = 8,
        perturbation = True,
        perturbation_object = args.perturbation_object,
        perturbation_value = args.perturbation_value,
        meta_file = files_dict["meta_file"],
        
    )
    # print(data_config)
    dl = data_config.init_dataloader()
    ds = data_config.init_dataset()

    # init chrombert
    model_config = ChromBERTFTConfig(
        genome = args.genome,
        dropout = 0,
        task = "general",
        pretrain_ckpt = files_dict["pretrain_ckpt"],
        mtx_mask = files_dict["mtx_mask"],
    )
    model_emb = model_config.init_model().get_embedding_manager().cuda().bfloat16()
    return ds, dl, model_emb


def model_emb_func(args,files_dict,odir):
    # init datamodule
    data_config = DatasetConfig(
        kind = "GeneralDataset",
        supervised_file = f"{odir}/model_input.tsv",
        hdf5_file = files_dict["hdf5_file"],
        batch_size = 4,
        num_workers = 8,
        perturbation = False,
        meta_file = files_dict["meta_file"],
        
    )
    dl = data_config.init_dataloader()
    ds = data_config.init_dataset()

    # init chrombert
    model_config = ChromBERTFTConfig(
        genome = args.genome,
        dropout = 0,
        task = "general",
        pretrain_ckpt = files_dict["pretrain_ckpt"],
        mtx_mask = files_dict["mtx_mask"],
    )
    model_emb = model_config.init_model().get_embedding_manager().cuda().bfloat16()
    return ds, dl, model_emb

def run(args, return_data=False):

    odir = args.odir
    os.makedirs(odir, exist_ok=True)

    focus_region = args.region
    files_dict = resolve_paths(args)
    check_files(files_dict, required_keys=[
        "chrombert_region_file",
        "meta_file",
        "hdf5_file",
        "pretrain_ckpt"])

    # ---------- overlapping focus regions ----------
    overlap_bed = check_region_file(focus_region,files_dict,odir)
    overlap_idx = overlap_bed["build_region_index"].to_numpy()

    # ---------- focus region embeddings (control)----------
    emb_npy_path = files_dict["region_emb_npy"]
    if os.path.exists(emb_npy_path):
        all_emb = np.load(emb_npy_path)
        region_embs = all_emb[overlap_idx]
        np.save(f"{odir}/{args.oname}_control.npy", region_embs)
    else:
        print(f"ChromBERT region embedding file not found (control): {emb_npy_path}, and not directly pick region embedding from cache dir.")
        print("Load model ChromBERT to embed focus regions (control).")
        ds, dl, model_emb = model_emb_func(args,files_dict,odir)

        region_embs = []
        with torch.no_grad():
            for batch in tqdm(dl, total = len(dl)):
                for k,v in batch.items():
                    if isinstance(v, torch.Tensor):
                        batch[k] = v.cuda()
                model_emb(batch) # initialize the cache 
                region_embs.append(model_emb.get_region_embedding().float().cpu().detach())
        region_embs = torch.cat(region_embs,axis=0).numpy()
        np.save(f"{odir}/{args.oname}_control.npy", region_embs)
    
    # ---------- focus region embeddings (perturbation)----------
    print("Load model ChromBERT to embed focus regions (perturbation).")
    ds, dl, model_emb = model_emb_func_perturb(args,files_dict,odir)

    perturbation_embs = []
    with torch.no_grad():
        for batch in tqdm(dl, total = len(dl)):
            for k,v in batch.items():
                if isinstance(v, torch.Tensor):
                    batch[k] = v.cuda()
            model_emb(batch) # initialize the cache 
            perturbation_embs.append(model_emb.get_region_embedding().float().cpu().detach())
    perturbation_embs = torch.cat(perturbation_embs,axis=0).numpy()
    np.save(f"{odir}/{args.oname}_perturb.npy", perturbation_embs)

    # ---------- report ----------
    total_focus = sum(1 for _ in open(focus_region))
    if os.path.exists(f"{odir}/no_overlap_region.bed"):
        no_overlap_region_len = sum(1 for _ in open(f"{odir}/no_overlap_region.bed"))
    else:
        no_overlap_region_len = 0

    print("Finished!")
    print(
        f"Focus region summary - total: {total_focus}, "
        f"overlapping with ChromBERT: {overlap_bed.shape[0]}, It is possible for a single region to overlap multiple ChromBERT regions,"
        f"non-overlapping: {no_overlap_region_len}"
    )
    print("Overlapping focus regions BED file:", f"{odir}/overlap_region.bed")
    print("Non-overlapping focus regions BED file:", f"{odir}/no_overlap_region.bed")
    print("Overlapping focus region embeddings (perturbation) saved to:", f"{odir}/{args.oname}_perturb.npy")
    print("Overlapping focus region embeddings (control) saved to:", f"{odir}/{args.oname}_control.npy")
    if return_data:
        return region_embs, overlap_bed


@click.command(name="embed_region_perturb",
               context_settings={"help_option_names": ["-h", "--help"]})
@click.option("--region", "region",
              type=click.Path(exists=True, dir_okay=False, readable=True),
              required=True, help="Region file.")
@click.option("--odir", default="./output", show_default=True,
              type=click.Path(file_okay=False), help="Output directory.")
@click.option("--oname", default="region_emb", show_default=True,
              type=str, 
              help="Output name of the region embeddings.")
@click.option("--perturbation-object", default="none", show_default=True,
              type=str, 
              help="Perturbation object. e.g. 'thap11;thap12;thap13'")
@click.option("--perturbation-value", default=0, show_default=True,
              type=float, 
              help="Perturbation value. e.g. 0, knock the TF binding to 0")
@click.option("--genome", default="hg38", show_default=True,
              type=click.Choice(["hg38", "mm10"], case_sensitive=False), help="Genome.")
@click.option("--resolution", default="1kb", show_default=True,
              type=click.Choice(["1kb", "200bp", "2kb", "4kb"], case_sensitive=False), help="Resolution. Mouse only supports 1kb resolution.")

@click.option("--chrombert-cache-dir", "chrombert_cache_dir",
              default="~/.cache/chrombert/data",
              show_default=True,
              type=click.Path(file_okay=False),
              help="ChromBERT cache dir. ")

@click.option("--chrombert-region-file", "chrombert_region_file",
              default=None,
              type=click.Path(exists=True, dir_okay=False, readable=True),
              help="ChromBERT region BED file. If not provided, use the default hg38_6k_1kb_region.bed in the cache dir.")
@click.option("--chrombert-region-emb-file", "chrombert_region_emb_file",
              default=None,
              type=click.Path(exists=True, dir_okay=False, readable=True),
              help="ChromBERT region embedding file. If not provided, use the default hm_1kb_all_region_emb.npy in the cache dir.")



def embed_region_perturb(region, odir, oname, genome, resolution, chrombert_cache_dir,chrombert_region_file, chrombert_region_emb_file, perturbation_object, perturbation_value):      
    '''
    Extract general region embeddings with perturbation
    '''
    args = SimpleNamespace(
        region=region,
        odir=odir,
        oname=oname,
        genome=genome,
        resolution=resolution,
        chrombert_cache_dir=chrombert_cache_dir,
        chrombert_region_file=chrombert_region_file,
        chrombert_region_emb_file=chrombert_region_emb_file,
        perturbation_object=perturbation_object,
        perturbation_value=perturbation_value
    )
    run(args)